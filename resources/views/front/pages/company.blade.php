...
@section('content')
    <div class="head-menu w-full px-8 md:px-48 block bg-[#242424]">
        <ul class="flex gap-2 md:gap-4">
            <li><a href=""
                    class="block text-sm md:text-lg leading-3 h-full text-white py-8 px-4 bg-gradient-to-r from-[#791119] to-[#BE1724]"
                    id="about">Hakkımızda</a></li>
            <li><a href="" class="block text-sm md:text-lg leading-3 h-full text-white py-8 px-4" id="whyus">Neden GAGA’yı
                    tercih etmeliyim?</a></li>
            <li><a href="" class="block text-sm md:text-lg leading-3 h-full text-white py-8 px-4" id="team">Ekibimiz</a>
            </li>
        </ul>
    </div>
    ...
    <div class="content px-8 md:px-48  py-8 md:py-24 bg-[#1B1B1B]">
        ...
        <div id="titles">
            ...
            <div class="mt-8 text-white text-base leading-7" id="description">
                {!!$setting->company_description!!}</div>
        </div>
                ...
                <a href="{{ route('team.detail', $people->slug) }}">
                    <div class="flex flex-col gap-4 group">
                        <img src="{{ asset('storage/peoples/' . $people->image) }}" alt="{{ $people->name }}"
                            class="rounded-2xl md:grayscale md:hover:grayscale-0 md:hover:-mt-8 transition-all duration-500">
...
@section('js')
    <script>
        ...
        $('#whyus').click(function(e) {
            ...
            $('#description').html('{!!$setting->whyus!!}');
            $('#peoples').hide();
            $('#titles').show();
        ...
        $('#about').click(function(e) {
            ...
            $('#description').html('{!!$setting->company_description!!}');
            $('#peoples').hide();
