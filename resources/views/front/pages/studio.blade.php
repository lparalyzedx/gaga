...
@section('content')
    <div class="content head-menu w-full px-8 md:px-48 block bg-[#242424]">
        <ul class="flex gap-2 md:gap-4">
            <li><a href="javascript:void(0)"
                    class="block text-sm md:text-lg leading-3  bg-gradient-to-r from-[#791119] to-[#BE1724] text-white py-8 px-4">Dönemsel
                    Workshoplar</a></li>
    ...
    <div
        ...
        <ul class="flex items-center justify-center" style="flex-wrap: wrap;">
            @foreach ($articles as $article)
                <li class="text-sm font-semibold mt-4 category"><a class="categorie" href="javascript:void(0)"
    ...
    <div class="content px-8 md:px-48  py-8 md:py-24 bg-[#1B1B1B]">
        <div class="flex flex-col md:flex-row">
            ...
            <div class="flex flex-col gap-8  basis-3/4 w-full px-8 md:px-24 py-8 md:py-24">
                <h1 class="text-2xl font-semibold text-white title">{{ $articled->title ?? '' }}</h1>
                <h3 class="text-base text-[#BE1724] tracking-[0.5em] date">{{ $articled->category->name ?? '' }} </h3>

                <div class="description text-white">{!! $articled->description !!}</div>
            </div>
            ...
            <div
                ...
                <ul class="text-center">
                    @foreach ($articles as $article)
                        <li class="text-xl font-semibold mt-4 category"><a class="categorie" style="margin-right: 3rem;"
                                href="javascript:void(0)" data-id="{{ $article->id }}">{{ $article->title }}</a></li>
                    @endforeach
                </ul>
            </div>
    ...
    <div id="carousel"
        class="flex md:grid gap-4 md:gap-8 md:grid-cols-3 px-16 md:px-48 py-16 md:py-24 bg-[#242424] rounded-xl">
        @foreach ($articled->images as $image)
            <img src="{{ asset('storage/articles/' . $image->image) }}" alt="" class="rounded-xl"
